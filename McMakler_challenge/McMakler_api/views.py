...
class McMacklerView(views.APIView):
    ...
    def get(self, request):
        ...
        while field:
            column = field.lower()  # Chnage paramter value into lower case
            queryset = Mcmackler.objects.values(column).order_by('id')  # create queryset based on column name in variable field
            ...
            if method == 'values':
                ...
                frequent_value = queryset.annotate(freq_value=Count(column)).order_by('-freq_value')[0]  # caluclate most frequent value in the queryset
